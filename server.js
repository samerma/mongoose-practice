...
Book.find({}).where('rating').lt(5).sort('-author').then(function (books) {
  //console.log(books)
})
//3. Find all the Fiction books, skip the first 2, and display only 3 of them
Book.find({}).where('genres').in(['Fiction']).skip(2).limit(3)
  .then(function (books) {
    //console.log(books)
  })
...
Person.find({}).where('height').gt(180).where('salary').gt(30000)
//.then(people => console.log(people))

//2. Find all the people who are tall (>180) OR rich (>30000)
//Person.find({}).where('height').gt(180).or('salary').gt(30000)
//.then(people => console.log(people))
Person.find({
  $or: [
    { 'height': { $gt: 180 } },
